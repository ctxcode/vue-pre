...
class ConvertJsExpression {
    ...
    public function parseValue($expr) {
        ...
        if (strpos($expr, '.length') > 0 && $this->match($lengthReg, $expr, $match)) {
            ...
            return "\Ctxkiwi\VuePre\ConvertJsExpression::length($value)"; // Make to return -1 instead of false
        }
        // Variable
        if ($this->match($varReg, $expr, $match)) {
            ...
            return $pre . '\Ctxkiwi\VuePre\ConvertJsExpression::getObjectValue($' . $varName . ', [' . implode(", ", $path) . '])';
        }
        // ( ... )
        if ($expr[0] === '(') {
        ...
        if (preg_match("/$opReg/", $expr) && $this->match("($valueReg)((?:$opReg$valueReg)+)", $expr, $match)) {
            ...
            foreach ($matches[0] as $amatch) {
                if ($this->match("($opReg)($valueReg)", $amatch, $subMatch)) {
                    ...
                    $op = trim($subMatch[1]);
                    if ($op === '+') {
                        $result = "\Ctxkiwi\VuePre\ConvertJsExpression::plus($result, $expr2)";
                    } else {
        ...
        if (strpos($expr, '.indexOf(') > 0 && $this->match($indexOfReg, $expr, $match)) {
            ...
            return "\Ctxkiwi\VuePre\ConvertJsExpression::indexOf($haystack, $needle)"; // Make to return -1 instead of false
        }
    ...
    public function match($regx, $expr, &$match, $options = []) {
        ...
        if (isset($options['all']) && $options['all']) {
            ...
            foreach ($matches as $k => $v) {
                ...
                $v = array_values(array_filter($v, '\Ctxkiwi\VuePre\ConvertJsExpression::filterRegexResults'));
                if (count($v) > 0) {
                    $match[] = $v;
                }
            }
            ...
            $match = array_values(array_filter($match, '\Ctxkiwi\VuePre\ConvertJsExpression::filterRegexResults'));
        }
